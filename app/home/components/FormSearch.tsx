    ...
    switch (e.key) {
      case 'ArrowRight':
        ...
        if (focusedDay === -1) {
          ...
          nextFocus =
            currentIndex < validDays.length - 1 ? validDays[currentIndex + 1] : validDays[0];
        }
        break;
      case 'ArrowLeft':
        e.preventDefault();
        if (focusedDay === -1) {
          ...
          const currentIndex = validDays.indexOf(focusedDay);
          nextFocus =
            currentIndex > 0 ? validDays[currentIndex - 1] : validDays[validDays.length - 1];
        }
        break;
      case 'ArrowDown':
        e.preventDefault();
                      ...
                      <div className={styles.eventMetadata}>
                        ...
                        <span className={styles.eventCategory}>
                          <span className='material-symbols-outlined'>
                            ...
                            {event.classifications?.genre?.toLowerCase().includes('alternative')
                              ? 'music_note'
                              : event.classifications?.genre?.toLowerCase().includes('comedy')
                                ? 'theater_comedy'
                                : event.classifications?.genre?.toLowerCase().includes('dance') ||
                                  ...
                                  : event.classifications?.genre
                                        ?.toLowerCase()
                                        .includes('hip-hop') ||
                                      event.classifications?.genre?.toLowerCase().includes('rap')
                                    ? 'keyboard_voice'
                                    : event.classifications?.genre?.toLowerCase().includes('metal')
                                      ? 'electric_bolt'
                                      : event.classifications?.genre
                                            ?.toLowerCase()
